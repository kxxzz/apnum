...
static void APNUM_intDigitsFromU32(APNUM_int* a, u32 u)
{
    ...
    while (u)
    {
        APNUM_Digit r = u % APNUM_Digit_Base;
        vec_push(a->digits, r);
        u /= APNUM_Digit_Base;
    }
}
...
static u32 APNUM_intFromStrWithHead(APNUM_pool_t pool, APNUM_int* out, u32 base, const char* str, u32 headLen)
{
    ...
    APNUM_int* b = APNUM_intNew(pool);
    APNUM_int* a1 = APNUM_intNew(pool);
...
)
{
    ...
    APNUM_int* q1 = APNUM_intNew(pool);
    APNUM_int* r = APNUM_intNew(pool);
    APNUM_int* ibase = APNUM_intNew(pool);
...
{
    APNUM_int* out = APNUM_intNew(pool);
    APNUM_intMul(pool, out, a, b);
    APNUM_intDup(a, out);
    APNUM_intFree(pool, out);
}
...
void APNUM_intDivSimple(APNUM_pool_t pool, APNUM_int* outQ, APNUM_int* outR, const APNUM_int* N, const APNUM_int* D)
{
    ...
    APNUM_int* RA = APNUM_intNew(pool);
    for (;;)
    {
        RA->neg = R->neg;
